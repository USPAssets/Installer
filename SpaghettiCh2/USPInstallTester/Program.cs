...
class TestInstallViewModel : InstallationViewModel
{
    ...
    public override bool AskUserQuestion(string message)
    {
        ...
        Console.WriteLine("(Assuming user selected 'yes')");
        return true;
    }
...
class Program
{
    ...
    static async Task<(string, string, bool)> GetConfig()
    {
        ...
        if (File.Exists(configPath))
        {
            ...
            if (lines.Length >= 3)
            {
                ...
                bool debugMod = lines[2].Trim().ToLower() == "true";
                Console.WriteLine("Install debug mod: " + debugMod);
                return (lines[0], lines[1], debugMod);
            }
        }
        ...
        string? debugModeInput = null;
        while (debugModeInput == null)
        {
            Console.Write("Install debug mod? [y/N]: ");
            debugModeInput = Console.ReadLine();
        }
        bool debugMode = debugModeInput.Trim().ToLower() == "y";
        await File.WriteAllLinesAsync(configPath, [assetPath, rootDir, debugMode.ToString()]);
        Console.WriteLine("Wrote configuration to: " + configPath);
        return (assetPath, rootDir, debugMode);
    }
    ...
    {
        (string assetPath, string rootDir, bool installDebug) = await GetConfig();
        ...
        foreach (string subDir in Directory.GetDirectories(rootDir))
        {
            try
            {
                ...
                if (deltaruneExe != null)
                {
                    ...
                    await installer.InstallDeltarune(assetPath, FindApplicationWithName(patchedCopyDir, "DELTARUNE")!, installDebug);
                }
            }
            catch (Exception e)
            {
